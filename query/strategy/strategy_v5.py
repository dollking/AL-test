...
from query.graph.vq_vae import VAE as vae
...
class Strategy(object):
    ...
    def train_by_epoch(self):
        ...
        for curr_it, data in enumerate(tqdm_batch):
            ...
            vq_loss, data_recon, _, encoding_indices = self.vae(data)
            ...
            indices_set |= set(encoding_indices.view([-1, ]).cpu().tolist())
        ...
        if self.epoch % 50 == 0:
            print(f'{self.epoch} - loss: {avg_loss.val} / best: {self.best} / centroid cnt: {len(indices_set)}')

    def get_index(self, inputs):
        self.vae.eval()
        with torch.no_grad():
            _, _, _, indices = self.vae(inputs)
