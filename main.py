...
from query.query_trans import Query
from query.strategy.strategy_ae import Strategy as autoencoder
from task.classification_trans import ClassificationWithTrans as Task
...
def main(cycle_cnt, config, query, ae):
    ...
    for step_cnt in range(config.max_cycle):
        # take a new sample
        query.sampling(step_cnt, task, ae)

        # train a task model
        task.run(query.labeled, ae)
